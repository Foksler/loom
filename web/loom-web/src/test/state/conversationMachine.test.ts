...
const conversationEventArb = fc.oneof(
  fc.record({ type: fc.constant('USER_INPUT' as const), content: fc.string({ minLength: 1, maxLength: 100 }) }),
  fc.record({ type: fc.constant('LLM_TEXT_DELTA' as const), content: fc.string({ minLength: 1, maxLength: 50 }) }),
  fc.record({ type: fc.constant('LLM_COMPLETED' as const), response: fc.constant({}) }),
  fc.record({ type: fc.constant('LLM_ERROR' as const), error: fc.string({ minLength: 1, maxLength: 50 }) }),
...
describe('conversationMachine', () => {
  ...
  test.prop([
    fc.array(
      fc.oneof(
        fc.record({ type: fc.constant('USER_INPUT' as const), content: fc.string({ minLength: 1, maxLength: 50 }) }),
        fc.record({ type: fc.constant('LLM_TEXT_DELTA' as const), content: fc.string({ minLength: 1, maxLength: 50 }) }),
        fc.record({ type: fc.constant('LLM_COMPLETED' as const), response: fc.constant({}) })
      ),
