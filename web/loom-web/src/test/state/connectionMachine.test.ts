...
const connectionEventArb = fc.oneof(
  fc.record({ type: fc.constant('CONNECT' as const), sessionId: fc.string({ minLength: 1, maxLength: 20 }) }),
  fc.record({ type: fc.constant('CONNECTED' as const) }),
  fc.record({ type: fc.constant('DISCONNECTED' as const), error: fc.option(fc.string(), { nil: undefined }) }),
...
describe('connectionMachine', () => {
   ...
   */
  test.prop([
    fc.array(connectionEventArb, { minLength: 0, maxLength: 20 }),
  ])('connected_resets_retry_count', (events) => {
    const actor = createActor(connectionMachine);
    actor.start();
