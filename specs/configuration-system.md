...
The Loom configuration system provides centralized, layered configuration management that allows users to customize behavior at multiple levels (system, user, workspace) while maintaining sensible defaults.
...
- **Clear Precedence:** Well-defined precedence rules for when the same setting is specified in multiple places
...
Loom follows the [XDG Base Directory Specification](https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html) for all file storage locations.
...
Configuration values are resolved using a layered precedence system. Higher precedence values override lower precedence values.
...
pub trait ConfigSource: Send + Sync {
    ...
    fn watch(&self) -> Option<Pin<Box<dyn Stream<Item = ConfigLayer> + Send>>> { None }
}
```
...
impl ConfigRegistry {
    ...
    pub fn register_source(&mut self, source: Box<dyn ConfigSource>);
    
    /// Load and resolve all configuration layers
    pub fn load(&self) -> Result<LoomConfig, ConfigError>;
    
    /// Get a watch channel for configuration changes
    pub fn watch(&self) -> tokio::sync::watch::Receiver<LoomConfig>;
    
    /// Force reload of all configuration
    pub fn reload(&self) -> Result<LoomConfig, ConfigError>;
}
```
...
```rust
impl ConfigLayer {
    /// Convert a partial configuration layer into a complete runtime configuration
    /// 
    /// This process:
...
pub fn load_config(cli_args: &CliArgs) -> Result<LoomConfig, ConfigError> {
    ...
    let mut layers: Vec<(u32, ConfigLayer)> = vec![
        (10, ConfigLayer::defaults()),
    ];
    ...
    pub fn defaults() -> Self {
        Self {
            ...
            providers: Some(HashMap::from([
                ("anthropic".to_string(), ProviderConfigLayer {
                ...
                ("openai".to_string(), ProviderConfigLayer {
                ...
                ("ollama".to_string(), ProviderConfigLayer {
...
*Reserved for future use when configuration schema changes require migration.*
