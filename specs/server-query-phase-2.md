...
**Scope:** LLM integration for automatic query detection and result injection into conversation context  
...
pub trait QueryDetectionStrategy: Send + Sync {
    ...
    fn detect_from_tool_call(&self, call: &ToolCall) 
        -> Option<ServerQuery>;
        ...
        for event in stream {
            match event {
                LlmEvent::TextDelta { content } => {
                    ...
                    if let Some(query) = self.detection_strategy.detect_from_text(
                        &accumulated_text,
                        conversation,
                    ) {
...
impl ResultFormatter for DefaultResultFormatter {
    async fn format_and_inject(
        ...
        let formatted = match &response.result {
            ...
            ServerQueryResult::CommandOutput { exit_code, stdout, stderr } => {
                format!(
...
fn test_result_formatting_file_content() {
    ...
    let message = rt.block_on(async {
        formatter.format_and_inject(&query, &response, &ConversationContext::default()).await
    }).unwrap();
...
async fn test_full_llm_query_flow() {
    ...
    let result = processor.process_with_queries(
        session_id,
...
async fn test_query_timeout_during_processing() {
    let processor = setup_processor_with_config(
        QueryDetectionConfig {
...
pub async fn batch_queries(
    ...
    let responses = self.wait_for_batch_responses(&batch.id, queries.len()).await?;
...
impl QueryResultCache {
    pub async fn get_or_fetch(
        ...
        // Store in cache
        self.cache.lock().await.insert(query_hash, CachedResult {
            query_hash,
...
Phase 2 transforms the Loom server-query system from a passive framework into an **active, intelligent query system** that:
