...
//!
//! 1. **Basic File Read**: LLM reads file, detector finds pattern, creates
//! query 2. **Environment + Context**: Multiple concurrent queries for
//! deployment info 3. **Error Handling**: Timeout and graceful recovery
//! mechanisms 4. **Concurrent Sessions**: Multiple clients without interference
					...
					if let Ok(val) = std::env::var(key) {
						...
						env_vars.insert(
							...
							match key.as_str() {
								...
								"DATABASE_URL" => "postgresql://user:pass@db.example.com/prod".to_string(),
								_ => format!("value_{}", key),
							},
						);
					}
...
///
/// **Why Important**: This is the most common query pattern. When an LLM
/// mentions a specific file, the server detects the pattern and reads the file
/// content to provide context for code modifications. This enables the LLM to
/// make intelligent edits based on the current file state.
///
/// **Flow**:
...
/// **Purpose**: Demonstrates querying multiple pieces of context
/// simultaneously.
///
