...
impl ServerQueryHandler for MockServerQueryHandler {
    async fn handle_query(
        ...
        match &query.kind {
            ...
            ServerQueryKind::Custom { name, .. } => Err(ServerQueryError::ProcessingFailed(
                format!("Unknown custom query: {}", name),
            )),
        }
    }
}
...
/// **Purpose**: Demonstrates querying environment variables and workspace context.
///
...
/// operations like deletion, the system can pause and ask for human confirmation
/// before proceeding.
...
/// **Purpose**: Demonstrates handling concurrent queries from different sessions.
///
