...
mod tests {
    ...
    // Property: GitDiff::is_empty returns true iff content is empty AND files_changed is empty.
    //
    // Why this test is important: The is_empty method is used to determine if there are
    // meaningful changes to commit. If either the content or the files_changed list has
    // data, the diff should NOT be considered empty. This property ensures the logic
    // correctly represents the "no changes" state.
    proptest! {
        #[test]
    ...
    ///
    /// Why this test is important: The Default trait implementation should produce
    /// an empty diff (no content, no files, zero stats). This is the natural starting
    /// state and must satisfy is_empty().
    #[test]
    ...
    ///
    /// Why this test is important: Even if no files are listed, having diff content
    /// means there are changes. This ensures partial diff information is still
    /// recognized as non-empty.
    #[test]
    ...
    ///
    /// Why this test is important: Even if the raw diff content is empty (e.g., only
    /// file mode changes), having files in the changed list means there are changes.
    #[test]
