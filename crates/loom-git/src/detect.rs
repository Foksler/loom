...
///
/// Returns `Ok(None)` if the path is not inside a git repository or git is not installed.
/// Returns `Ok(Some(metadata))` with branch and remote information if available.
...
/// Returns the URL of the default remote (origin, upstream, or first available).
...
mod tests {
    ...
    ///
    /// Why this test is important: The function must gracefully handle directories
    /// that are not git repositories, returning None instead of an error. This is
    /// the most common case when scanning arbitrary directories.
    #[test]
    ...
    /// conventional default and should be preferred. This matches user expectations
    /// and git's own behavior.
    #[test]
    ...
    /// the exact state of the codebase. We must return the full SHA (not abbreviated)
    /// to ensure unambiguous commit identification across large repositories.
    #[test]
    ...
    /// a clean state before operations. This tests both untracked and modified files.
    #[test]
    ...
    ///
    /// Why this test is important: This is the main integration point that combines
    /// all repository detection features. It must correctly aggregate branch, remote,
    /// commit, and dirty state into a single coherent status object for consumers.
    #[test]
