...
mod tests {
    ...
    ///
    /// Why this test is important: We must correctly identify directories that are
    /// NOT git repositories to avoid attempting git operations on them, which would
    ...
    ///
    /// Why this test is important: After a commit, the working tree should be clean
    /// and diff_all should return an empty diff. This is the expected quiescent state.
    #[tokio::test]
    ...
    /// commit, so we cannot diff against HEAD. The implementation must handle this
    /// edge case by diffing staged changes instead.
    #[tokio::test]
    ...
    /// files were modified. This verifies correct extraction from typical stat output.
    #[test]
