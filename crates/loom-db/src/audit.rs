...
impl AuditRepository {
	...
	pub async fn log_event(&self, entry: &AuditLogEntry) -> Result<(), DbError> {
		let now = Utc::now();
	...
	pub async fn cleanup_old_logs(&self) -> Result<i64, DbError> {
...
fn parse_audit_row(row: &sqlx::sqlite::SqliteRow) -> Result<AuditLogEntry, DbError> {
	...
	let id = Uuid::parse_str(&id_str)
		.map_err(|e| DbError::Internal(format!("Invalid audit log id UUID: {e}")))?;
	...
	let event_type: AuditEventType = serde_json::from_value(serde_json::Value::String(event_type_str.clone()))
		.map_err(|e| DbError::Internal(format!("Invalid event_type '{event_type_str}': {e}")))?;

	let actor_user_id = actor_user_id_str
		.map(|s| {
			Uuid::parse_str(&s)
				.map(UserId::new)
				.map_err(|e| DbError::Internal(format!("Invalid actor_user_id UUID: {e}")))
		})
		.transpose()?;
	...
	let impersonating_user_id = impersonating_user_id_str
		.map(|s| {
			Uuid::parse_str(&s)
				.map(UserId::new)
				.map_err(|e| DbError::Internal(format!("Invalid impersonating_user_id UUID: {e}")))
		})
		.transpose()?;
