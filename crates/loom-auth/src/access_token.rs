...
mod tests {
    ...
    mod token_generation {
        ...
        fn generates_token_with_correct_length() {
            ...
            assert_eq!(token.len(), ACCESS_TOKEN_PREFIX.len() + ACCESS_TOKEN_BYTES * 2);
        }
    ...
    mod access_token_struct {
        ...
        fn new_creates_token_with_60_day_expiry() {
            ...
            let diff = (access_token.expires_at - expected_expiry).num_seconds().abs();
        ...
        fn verify_works_with_stored_token() {
            let (access_token, plaintext) = AccessToken::new(UserId::generate(), "Test", SessionType::Cli);
