...
pub fn render_email(template: &EmailTemplate, locale: &str) -> (String, String) {
    ...
    match template {
        ...
        EmailTemplate::SecurityNotification { event, details } => {
            let subject = t_fmt(locale, "server.email.security_alert.subject", &[
                ("event", event)
            ]);
            let body = format!(
                "{}\n\n{}\n\n{}\n{}\n\n{}",
                t(locale, "server.email.security_alert.body"),
                t_fmt(locale, "server.email.security_alert.event", &[("event", event)]),
        ...
        EmailTemplate::OrgInvitation {
            ...
            let subject = t_fmt(locale, "server.email.invitation.subject", &[
                ("org_name", org_name)
            ]);
            let body = format!(
                "{}\n\n{}\n\nhttps://loom.example/api/invitations/{}/accept\n\n{}",
                t_fmt(locale, "server.email.invitation.body", &[
        ...
        EmailTemplate::AccountDeletionWarning { days_remaining } => {
            ...
            let body = format!(
                "{}\n\n{}\n\n{}",
                t_fmt(locale, "server.email.deletion_warning.body", &[
