...
mod tests {
    ...
    /// Property test: ToolExecutionStatus.call_id() always returns a consistent value
    /// regardless of which variant we're in.
    ///
    /// This test verifies that the call_id accessor correctly extracts the call_id
    /// from any variant, ensuring we don't have bugs in pattern matching.
    #[test]
    ...
    /// Property test: AgentState.name() returns a non-empty string for all variants.
    ///
