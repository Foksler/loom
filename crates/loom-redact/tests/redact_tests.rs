...
fn test_aws_access_token() {
    ...
    let result = redact(&input);
    assert!(result.contains("REDACTED:aws-access-token"), "Result: {}", result);
}
...
fn test_github_fine_grained_pat() {
    let token = format!("github_pat_{}", "11ABCDEFG0123456789_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abc");
    let input = format!("token: {}", token);
    let result = redact(&input);
    assert!(result.contains("REDACTED:github-fine-grained-pat"), "Result: {}", result);
}
...
fn test_entropy_filtering_low_entropy() {
    ...
    let result = redact(input);
    assert!(!result.contains("REDACTED:"), "Low entropy should not match: {}", result);
}
...
#[test]
fn test_slack_token() {
    let token = format!("xoxb-{}-{}-{}", "1234567890123", "1234567890123", "AbCdEfGhIjKlMnOpQrStUvWx");
...
fn test_private_key_detection() {
    ...
    let result = redact(input);
    assert!(result.contains("REDACTED:private-key"), "Result: {}", result);
}
...
fn test_huggingface_token() {
    ...
    let result = redact(&input);
    assert!(result.contains("REDACTED:huggingface"), "Result: {}", result);
}
...
fn test_linear_api_key() {
    ...
    let result = redact(&input);
    assert!(result.contains("REDACTED:linear-api-key"), "Result: {}", result);
}

#[test]
fn test_sendgrid_api_key() {
    let token = format!("SG.{}", "abcdefghij1234567890abcdefghij1234567890abcdefghij1234567890abcdef");
