...
impl ShareRepository {
	...
	pub async fn create_share_link(&self, share_link: &ShareLink) -> Result<(), ServerError> {
		sqlx::query(
			r#"
	...
	pub async fn get_share_link_by_thread(
		&self,
		thread_id: &str,
	) -> Result<Option<ShareLink>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	#[tracing::instrument(skip(self), fields(thread_id = %thread_id))]
	pub async fn revoke_share_link(&self, thread_id: &str) -> Result<i64, ServerError> {
		let now = Utc::now();
	...
	pub async fn get_pending_support_access(
		&self,
		thread_id: &str,
	) -> Result<Option<SupportAccess>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn revoke_support_access(&self, id: &Uuid) -> Result<bool, ServerError> {
		let now = Utc::now();
...
fn parse_share_link_row(row: &sqlx::sqlite::SqliteRow) -> Result<ShareLink, ServerError> {
	...
	let id = Uuid::parse_str(&id_str)
		.map_err(|e| ServerError::Internal(format!("Invalid share_link id UUID: {e}")))?;
...
fn parse_support_access_row(row: &sqlx::sqlite::SqliteRow) -> Result<SupportAccess, ServerError> {
	let id_str: String = row.get("id");
	let thread_id: String = row.get("thread_id");
	...
	let id = Uuid::parse_str(&id_str)
		.map_err(|e| ServerError::Internal(format!("Invalid support_access id UUID: {e}")))?;
	let requested_by = Uuid::parse_str(&requested_by_str)
		.map_err(|e| ServerError::Internal(format!("Invalid requested_by UUID: {e}")))?;

	let approved_by = approved_by_str
		.map(|s| {
			Uuid::parse_str(&s)
				.map(UserId::new)
				.map_err(|e| ServerError::Internal(format!("Invalid approved_by UUID: {e}")))
		})
		.transpose()?;

	let requested_at = DateTime::parse_from_rfc3339(&requested_at_str)
		.map_err(|e| ServerError::Internal(format!("Invalid requested_at: {e}")))?
		.with_timezone(&Utc);
	...
	let approved_at = approved_at_str
		.map(|s| {
			DateTime::parse_from_rfc3339(&s)
				.map(|dt| dt.with_timezone(&Utc))
				.map_err(|e| ServerError::Internal(format!("Invalid approved_at: {e}")))
		})
		.transpose()?;
