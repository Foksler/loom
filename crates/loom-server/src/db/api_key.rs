...
impl ApiKeyRepository {
	...
	pub async fn create_api_key(
		...
		let scopes_json = serde_json::to_string(scopes)
			.map_err(|e| ServerError::Internal(format!("Failed to serialize scopes: {e}")))?;
	...
	pub async fn get_api_key_by_id(&self, id: &str) -> Result<Option<ApiKey>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn get_api_key_by_hash(&self, token_hash: &str) -> Result<Option<ApiKey>, ServerError> {
		let row = sqlx::query(
			r#"
			SELECT id, org_id, name, token_hash, scopes, created_by, created_at,
			       last_used_at, revoked_at, revoked_by
	...
	pub async fn list_api_keys_for_org(&self, org_id: &OrgId) -> Result<Vec<ApiKey>, ServerError> {
		let rows = sqlx::query(
			r#"
			SELECT id, org_id, name, token_hash, scopes, created_by, created_at,
			       last_used_at, revoked_at, revoked_by
	...
	pub async fn revoke_api_key(&self, id: &str, revoked_by: &UserId) -> Result<bool, ServerError> {
		let now = Utc::now();
	...
	/// * `id` - The API key's UUID
	#[tracing::instrument(skip(self), fields(api_key_id = %id))]
	pub async fn update_last_used(&self, id: &str) -> Result<(), ServerError> {
		let now = Utc::now();
	...
	pub async fn get_usage_logs(
		...
		limit: i32,
		offset: i32,
	) -> Result<(Vec<ApiKeyUsage>, i64), ServerError> {
...
fn parse_api_key_row(row: &sqlx::sqlite::SqliteRow) -> Result<ApiKey, ServerError> {
	let id_str: String = row.get("id");
	let org_id_str: String = row.get("org_id");
	let name: String = row.get("name");
	// Note: token_hash is sensitive, we retrieve it but should not log it
	...
	let id = Uuid::parse_str(&id_str)
		.map_err(|e| ServerError::Internal(format!("Invalid api_key id UUID: {e}")))?;
	let org_id = Uuid::parse_str(&org_id_str)
		.map_err(|e| ServerError::Internal(format!("Invalid org_id UUID: {e}")))?;
	let created_by = Uuid::parse_str(&created_by_str)
		.map_err(|e| ServerError::Internal(format!("Invalid created_by UUID: {e}")))?;

	let scopes: Vec<ApiKeyScope> = serde_json::from_str(&scopes_json)
		.map_err(|e| ServerError::Internal(format!("Invalid scopes JSON: {e}")))?;
	...
	let revoked_at = revoked_at_str
		.map(|s| {
			DateTime::parse_from_rfc3339(&s)
				.map(|dt| dt.with_timezone(&Utc))
				.map_err(|e| ServerError::Internal(format!("Invalid revoked_at: {e}")))
		})
		.transpose()?;
	...
	let revoked_by = revoked_by_str
		.map(|s| {
			Uuid::parse_str(&s)
				.map(UserId::new)
				.map_err(|e| ServerError::Internal(format!("Invalid revoked_by UUID: {e}")))
		})
		.transpose()?;
...
fn parse_api_key_usage_row(row: &sqlx::sqlite::SqliteRow) -> Result<ApiKeyUsage, ServerError> {
	let id_str: String = row.get("id");
	...
	let id = Uuid::parse_str(&id_str)
		.map_err(|e| ServerError::Internal(format!("Invalid usage id UUID: {e}")))?;
	let api_key_id = Uuid::parse_str(&api_key_id_str)
		.map_err(|e| ServerError::Internal(format!("Invalid api_key_id UUID: {e}")))?;
	...
	let ip_address: Option<IpAddr> = ip_address_str
		.map(|s| {
			s.parse()
				.map_err(|e| ServerError::Internal(format!("Invalid IP address: {e}")))
		})
		.transpose()?;
