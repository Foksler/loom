...
impl TeamRepository {
	...
	///
	/// # Errors
	/// Returns `ServerError::Database` if insert fails (e.g., duplicate slug within org).
	///
	...
	pub async fn create_team(&self, team: &Team) -> Result<(), ServerError> {
		sqlx::query(
			r#"
	...
	pub async fn get_team_by_id(&self, id: &TeamId) -> Result<Option<Team>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn get_team_by_slug(
		...
		org_id: &OrgId,
		slug: &str,
	) -> Result<Option<Team>, ServerError> {
		let row = sqlx::query(
			r#"
			SELECT id, org_id, name, slug, created_at, updated_at
			FROM teams
	...
	pub async fn update_team(&self, team: &Team) -> Result<(), ServerError> {
	...
	#[tracing::instrument(skip(self), fields(team_id = %id))]
	pub async fn delete_team(&self, id: &TeamId) -> Result<bool, ServerError> {
	...
	pub async fn list_teams_for_org(&self, org_id: &OrgId) -> Result<Vec<Team>, ServerError> {
		let rows = sqlx::query(
			r#"
			SELECT id, org_id, name, slug, created_at, updated_at
			FROM teams
	...
	pub async fn get_membership(
		...
		team_id: &TeamId,
		user_id: &UserId,
	) -> Result<Option<TeamMembership>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn list_members(&self, team_id: &TeamId) -> Result<Vec<TeamMembership>, ServerError> {
		let rows = sqlx::query(
			r#"
			SELECT id, team_id, user_id, role, created_at
			FROM team_memberships
	...
	pub async fn get_teams_for_user(
		&self,
		user_id: &UserId,
	) -> Result<Vec<(Team, TeamRole)>, ServerError> {
		let rows = sqlx::query(
			r#"
	...
	fn row_to_team(&self, row: &sqlx::sqlite::SqliteRow) -> Result<Team, ServerError> {
		...
		let id = Uuid::parse_str(&id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid team ID: {e}")))?;
	...
	fn row_to_membership(
	...
	) -> Result<TeamMembership, ServerError> {
		let id_str: String = row.get("id");
		...
		let id = Uuid::parse_str(&id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid membership ID: {e}")))?;
		let team_id = Uuid::parse_str(&team_id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid team_id: {e}")))?;
