...
impl OrgRepository {
	...
	///
	/// # Errors
	/// Returns `ServerError::Database` if insert fails (e.g., duplicate slug).
	///
	/// # Database Constraints
	/// - `id` must be unique
	/// - `slug` must be unique
	#[tracing::instrument(skip(self, org), fields(org_id = %org.id, slug = %org.slug))]
	pub async fn create_org(&self, org: &Organization) -> Result<(), ServerError> {
	...
	pub async fn get_org_by_id(&self, id: &OrgId) -> Result<Option<Organization>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn get_org_by_id_including_deleted(
		&self,
		id: &OrgId,
	) -> Result<Option<Organization>, ServerError> {
		let row = sqlx::query(
			r#"
			SELECT id, name, slug, visibility, is_personal, created_at, updated_at, deleted_at
			FROM organizations
	...
	pub async fn get_org_by_slug(&self, slug: &str) -> Result<Option<Organization>, ServerError> {
		let row = sqlx::query(
			r#"
			SELECT id, name, slug, visibility, is_personal, created_at, updated_at, deleted_at
			FROM organizations
	...
	///
	/// # Errors
	/// Returns `ServerError::Database` if update fails (e.g., duplicate slug).
	#[tracing::instrument(skip(self, org), fields(org_id = %org.id))]
	pub async fn update_org(&self, org: &Organization) -> Result<(), ServerError> {
		let now = Utc::now().to_rfc3339();
		sqlx::query(
			r#"
	...
	/// * `id` - The organization's UUID
	#[tracing::instrument(skip(self), fields(org_id = %id))]
	pub async fn soft_delete_org(&self, id: &OrgId) -> Result<(), ServerError> {
		let now = Utc::now().to_rfc3339();
		sqlx::query(
			r#"
			UPDATE organizations
	...
	/// * `id` - The organization's UUID
	#[tracing::instrument(skip(self), fields(org_id = %id))]
	pub async fn restore_org(&self, id: &OrgId) -> Result<(), ServerError> {
		let now = Utc::now().to_rfc3339();
		sqlx::query(
			r#"
			UPDATE organizations
	...
	pub async fn list_orgs_for_user(
		&self,
		user_id: &UserId,
	) -> Result<Vec<Organization>, ServerError> {
		let rows = sqlx::query(
			r#"
	...
	pub async fn get_membership(
		...
		org_id: &OrgId,
		user_id: &UserId,
	) -> Result<Option<OrgMembership>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn list_members(
		&self,
		org_id: &OrgId,
	) -> Result<Vec<(OrgMembership, User)>, ServerError> {
		let rows = sqlx::query(
			r#"
			SELECT 
	...
	#[tracing::instrument(skip(self), fields(org_id = %org_id))]
	pub async fn count_owners(&self, org_id: &OrgId) -> Result<i64, ServerError> {
	...
	pub async fn get_invitation_by_token_hash(
		&self,
		token_hash: &str,
	) -> Result<Option<OrgInvitation>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn accept_invitation(&self, id: &str) -> Result<(), ServerError> {
		let now = Utc::now().to_rfc3339();
		sqlx::query(
			r#"
	...
	#[tracing::instrument(skip(self), fields(invitation_id = %id))]
	pub async fn delete_invitation(&self, id: &str) -> Result<bool, ServerError> {
		let result = sqlx::query(
			r#"
	...
	pub async fn list_pending_invitations(
		&self,
		org_id: &OrgId,
	) -> Result<Vec<OrgInvitation>, ServerError> {
		let now = Utc::now().to_rfc3339();
		let rows = sqlx::query(
			r#"
			SELECT id, org_id, email, role, invited_by, token_hash, created_at, expires_at, accepted_at
	...
	pub async fn get_join_request(&self, id: &str) -> Result<Option<OrgJoinRequest>, ServerError> {
		let row = sqlx::query(
			r#"
	...
	pub async fn list_pending_join_requests(
		&self,
		org_id: &OrgId,
	) -> Result<Vec<OrgJoinRequest>, ServerError> {
		let rows = sqlx::query(
			r#"
			SELECT org_id, user_id, created_at, handled_at, handled_by, approved
			FROM org_join_requests
	...
	pub async fn list_pending_join_requests_with_users(
		&self,
		org_id: &OrgId,
	) -> Result<Vec<(OrgJoinRequest, User)>, ServerError> {
		let rows = sqlx::query(
			r#"
			SELECT 
	...
	fn row_to_org(&self, row: &sqlx::sqlite::SqliteRow) -> Result<Organization, ServerError> {
		...
		let id = Uuid::parse_str(&id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid org ID: {e}")))?;
	...
	fn row_to_membership(
	...
	) -> Result<OrgMembership, ServerError> {
		...
		let org_id = Uuid::parse_str(&org_id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid org_id: {e}")))?;
		let user_id = Uuid::parse_str(&user_id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid user_id: {e}")))?;
	...
	fn row_to_user_prefixed(&self, row: &sqlx::sqlite::SqliteRow) -> Result<User, ServerError> {
	...
	fn row_to_invitation(
		&self,
		row: &sqlx::sqlite::SqliteRow,
	) -> Result<OrgInvitation, ServerError> {
		let id_str: String = row.get("id");
		let org_id_str: String = row.get("org_id");
		...
		let id = Uuid::parse_str(&id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid invitation ID: {e}")))?;
		let org_id = Uuid::parse_str(&org_id_str)
			.map_err(|e| ServerError::Internal(format!("Invalid org_id: {e}")))?;
		let invited_by = Uuid::parse_str(&invited_by_str)
			.map_err(|e| ServerError::Internal(format!("Invalid invited_by: {e}")))?;
		let role = match role_str.as_str() {
			"owner" => OrgRole::Owner,
			"admin" => OrgRole::Admin,
			_ => OrgRole::Member,
	...
	fn row_to_join_request(
		&self,
		row: &sqlx::sqlite::SqliteRow,
	) -> Result<OrgJoinRequest, ServerError> {
		let org_id_str: String = row.get("org_id");
		let user_id_str: String = row.get("user_id");
		let created_at: String = row.get("created_at");
