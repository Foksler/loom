...
//!
//! Provides comprehensive tracing of server-to-client queries for debugging and performance analysis.
//! Each query gets a unique trace ID and events are recorded chronologically.
//!
//! # Example
//!
//! ```
//! use loom_server::query_tracing::{QueryTracer, QueryTraceStore};
...
pub struct TraceEvent {
    ...
    /// Event type (e.g., "created", "sent", "response_received", "error", "timeout")
...
impl QueryTracer {
    ...
    pub fn event_duration(&self, from_index: usize, to_index: usize) -> Option<Duration> {
        if from_index >= self.events.len()
            || to_index >= self.events.len()
            || from_index >= to_index
        {
            return None;
        }
