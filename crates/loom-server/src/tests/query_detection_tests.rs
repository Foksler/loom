...
use crate::llm_query_handler::SimpleRegexDetector;
...
mod tests {
	...
	/// Test detection of multiple queries in single output.
	/// **Purpose**: Ensures the detector doesn't stop at the first query and can
	/// handle complex LLM outputs requesting multiple operations. Critical for
	/// sequential workflows requiring multiple file reads or commands.
	#[test]
	fn test_multiple_queries_in_single_output() {
		let detector = SimpleRegexDetector::new();

		let output = "I need to read config.json and then execute cargo build";
