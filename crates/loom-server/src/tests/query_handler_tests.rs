...
use crate::llm_query_handler::{LlmQueryHandler, SimpleRegexDetector};
...
mod tests {
    ...
    async fn test_concurrent_queries_same_session() {
        ...
        let task1 =
            tokio::spawn(async move { handler.handle_llm_output("session-1", output1).await });

        let task2 =
            tokio::spawn(
                async move { handler_clone.handle_llm_output("session-1", output2).await },
            );
        ...
        // Both tasks should complete (either successfully or timeout)
        ...
        assert!(result1.is_ok(), "Task1 should complete");
        assert!(result2.is_ok(), "Task2 should complete");
    }

    /// Test concurrent queries on different sessions.
    /// **Purpose**: Queries from different sessions must not interfere or
    /// receive responses meant for other sessions.
    #[tokio::test]
    async fn test_concurrent_queries_different_sessions() {
        ...
        let task1 =
            tokio::spawn(async move { handler.handle_llm_output("session-1", output).await });

        let task2 =
            tokio::spawn(async move { handler_clone.handle_llm_output("session-2", output).await });

        // Give time for queries to be sent
        tokio::time::sleep(Duration::from_millis(100)).await;

        // Get all pending queries
        let all_pending = manager_clone.list_pending("session-1").await;

        // Send responses to all pending queries
        for query in all_pending {
            let response = ServerQueryResponse {
                query_id: query.id.clone(),
                sent_at: chrono::Utc::now().to_rfc3339(),
                result: ServerQueryResult::FileContent("session content".to_string()),
                error: None,
            };
            manager_clone.receive_response(response).await;
        }

        // Both should eventually complete or timeout
        let result1 = tokio::time::timeout(Duration::from_secs(15), task1).await;
        let result2 = tokio::time::timeout(Duration::from_secs(15), task2).await;
    ...
    // ============================================================================
    // Query Response Injection Tests
    // ============================================================================

    /// Test that query responses are correctly returned from handle_llm_output.
