...
#[cfg(test)]
mod tests {
    use crate::query_tracing::{QueryTraceStore, QueryTracer, TraceTimeline};
    ...
    fn test_tracer_creation_event() {
        ...
        assert_eq!(
            tracer.next_sequence, 1,
            "Next sequence should be 1 after creation"
        );
    }
    ...
    async fn test_trace_overhead_minimal() {
        ...
        for i in 0..1000 {
            let mut tracer =
                QueryTracer::new(format!("Q-perf-{}", i), Some("perf-test".to_string()));
            tracer.record_sent(5);
            tracer.record_response_received("ok");
            store.store(tracer).await;
        }
