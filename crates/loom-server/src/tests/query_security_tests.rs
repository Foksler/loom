...
mod tests {
	...
	fn test_sanitizer_accepts_valid_relative_paths() {
		...
		assert!(result.is_ok(), "Current directory should be accepted");

		if let Ok(path) = result {
			assert!(
				path.starts_with("/tmp"),
				"Should be joined with workspace root"
			);
		}
	}
	...
	/// **Why Important**: Valid JSON should always be accepted.
	#[test]
	fn test_json_validation_valid_json() {
		let validator = QueryValidator::new();

		let valid_json = r#"{"key": "value", "number": 42}"#;
		let result = validator.validate_json(valid_json);
		assert!(result.is_ok(), "Valid JSON should pass");
	}

	/// Test that invalid JSON is rejected.
	/// **Why Important**: Malformed JSON could cause parsing errors or injection.
	#[test]
	fn test_json_validation_invalid_json() {
		let validator = QueryValidator::new();
	...
	/// Test that rate limiter allows queries within rate.
	/// **Why Important**: Legitimate queries should not be blocked. Rate limits
	/// should only trigger after exceeding the configured rate.
	#[tokio::test]
	async fn test_rate_limiter_allows_within_rate() {
		...
		for i in 0..10 {
			...
			assert!(
				result.is_ok(),
				"Request {} should be allowed within rate",
				i + 1
			);
		}
	}
	...
	/// **Why Important**: Rate limiting must prevent DoS attacks by blocking
	/// excessive queries from a single session.
	#[tokio::test]
	async fn test_rate_limiter_blocks_when_exceeded() {
		let limiter = RateLimiter::with_rate(1.0, 2); // 2 tokens max, 1 token/sec refill
