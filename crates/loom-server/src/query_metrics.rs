...
//!
//! This module provides Prometheus metrics for tracking server query operations,
//! including throughput, latency, success/failure rates, and pending query count.

use prometheus::{
    Counter, CounterVec, Encoder, Gauge, Histogram, HistogramVec, IntGaugeVec, Registry,
    TextEncoder,
};
use std::sync::Arc;
use tracing::error;
...
impl QueryMetrics {
    ...
    pub fn record_failure(&self, query_type: &str, error_type: &str, session_id: &str) {
        ...
        self.queries_failure_by_type
            .with_label_values(&[query_type, error_type])
            .inc();

        if error_type == "timeout" {
            self.query_timeouts_total
                .with_label_values(&[query_type])
                .inc();
        }
