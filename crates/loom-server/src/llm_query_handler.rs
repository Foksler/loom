...
//!
//! This module detects when LLM output contains queries and automatically processes
//! them through the ServerQueryManager. It uses pattern matching strategies to identify
//! query requests embedded in LLM text output.
//!
//! ## Architecture
//!
//! - `QueryDetector`: Trait for pluggable query detection implementations
//! - `SimpleRegexDetector`: Pattern matching based detector for common query phrases
...
impl SimpleRegexDetector {
    ...
    ///
    /// # Panics
    /// Panics if regex patterns are invalid (should not happen with static patterns).
    pub fn new() -> Self {
        Self {
...
impl LlmQueryHandler {
    ...
    ///
    /// # Returns
    /// - `Ok(Some(response))` - A query was detected and processed, response is ready
    ...
    ///
    /// # Returns
    /// A vector of responses for all detected queries, in the order they were detected.
    #[instrument(
        skip(self, llm_output),
        fields(
            session_id = %session_id,
...
mod tests {
    ...
    /// **Why Important**: Ensures the detector correctly extracts file paths from various
    /// natural language patterns. File path extraction is critical for ReadFile queries.
    #[tokio::test]
    ...
    /// **Why Important**: Ensures command and argument parsing works correctly for command
    /// execution requests. This prevents misinterpreted commands from being executed.
    #[tokio::test]
    ...
    /// **Why Important**: Environment variable parsing must correctly identify and uppercase
    /// variable names to match actual environment variables. Incorrect parsing could cause
    /// the wrong environment data to be sent to the LLM.
    #[tokio::test]
    ...
    /// **Why Important**: Ensures prompt extraction from natural language works correctly.
    /// Malformed prompts could confuse users or indicate a detector issue.
    #[tokio::test]
    ...
    /// **Why Important**: Ensures the detector doesn't stop after finding the first query
    /// and can handle complex LLM outputs requesting multiple operations.
    #[tokio::test]
    ...
    /// **Why Important**: The detector must gracefully handle edge cases and invalid input
    /// without panicking. This ensures robustness in production.
    #[tokio::test]
    ...
    /// **Why Important**: Path normalization ensures consistency in file operations.
    /// Paths must be in expected format for the client to handle them correctly.
    #[test]
    ...
    /// **Why Important**: Command arguments must be parsed and separated correctly to
    /// prevent injection issues and ensure proper command execution.
    #[test]
    ...
    /// **Why Important**: Environment variable names must be correctly parsed and uppercased.
    /// Invalid parsing could request non-existent variables or miss requested ones.
    #[test]
    ...
    /// **Why Important**: Query IDs are used for correlation between requests and responses.
    /// Invalid IDs could break the query-response matching mechanism.
    #[tokio::test]
    ...
    /// **Why Important**: The handler must gracefully return None when no queries are present,
    /// allowing normal LLM flow to continue without interruption.
    #[tokio::test]
    ...
    /// **Why Important**: The convenience constructor ensures users don't need to manually
    /// create and wire up dependencies for typical use cases.
    #[test]
    ...
    /// **Why Important**: Metadata aids debugging and tracing query origins through the system.
    /// Missing or incorrect metadata makes troubleshooting difficult.
    #[tokio::test]
    ...
    /// **Why Important**: Different query types have different expected response times.
    /// Incorrect timeouts could cause acceptable requests to fail or cause excessive waiting.
    #[tokio::test]
