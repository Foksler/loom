...
impl SecretsConfig {
    ...
    pub fn from_env() -> SecretsResult<Self> {
        ...
        let svid_issuer = std::env::var("LOOM_SECRETS_SVID_ISSUER").unwrap_or_else(|_| "loom".to_string());
...
mod tests {
    ...
    fn config_uses_default_svid_ttl() {
        ...
        .unwrap();
        assert_eq!(config.svid_ttl(), Duration::from_secs(DEFAULT_SVID_TTL_SECS));
    }
