//! Oracle tool that queries OpenAI for additional reasoning/advice via the Loom server proxy.
...
const DEFAULT_SYSTEM_PROMPT: &str = "You are the 'oracle' sub-agent for Loom. You are being called by another AI model (Claude) to provide additional reasoning or advice. Answer concisely and focus on technical correctness and clarity.";
...
impl RetryableError for OracleError {
    fn is_retryable(&self) -> bool {
        match self {
            ...
            OracleError::Http { status, .. } => {
                matches!(*status, 429 | 408 | 502 | 503 | 504)
            }
        }
    }
}
...
impl Default for OracleTool {
    fn default() -> Self {
        let base_url = std::env::var("LOOM_SERVER_URL")
...
impl Tool for OracleTool {
    ...
    fn description(&self) -> &str {
        "Query OpenAI for additional reasoning or advice via the Loom server proxy. Use this tool when you need a second opinion, help with complex reasoning, or specialized knowledge."
    }
